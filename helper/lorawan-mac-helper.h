...
public:
  ...
  //Added by BC
  static int SetSpreadingFactorsUpBasedOnDistanceAlone (NodeContainer endDevices, NodeContainer gateways,
                                                 Ptr<LoraChannel> channel);

  // Added by BC
  // Set all Spreading Factors to 7
  static std::vector<int> SetAllSpreadingFactorsToX (NodeContainer endDevices, NodeContainer gateways,
                                           Ptr<LoraChannel> channel, int SF);

  // Added by BC
  // Set Spreading Factor of single node to X
  static int SetSpreadingFactorToX (NodeContainer endDevices, NodeContainer gateways,
  ...
  int SetDistributedSpreadingFactorsUpwards (NodeContainer endDevices, NodeContainer gateways, Ptr<LoraChannel> channel, std::vector<int> SFdistribution);
  int SetDistributedSpreadingFactorsAtRandom (NodeContainer endDevices, NodeContainer gateways, Ptr<LoraChannel> channel, std::vector<int> SFdistribution);
  int SetDistributedSpreadingFactorsDownwards (NodeContainer endDevices, NodeContainer gateways, Ptr<LoraChannel> channel, std::vector<int> SFdistribution);

  static std::vector<int>GetSFDistribution (NodeContainer endDevices);

  void PrintSFDistribution(NodeContainer endDevices);
  void PrintSFDistributionToFile(NodeContainer endDevices ,std::string filename);
private:
  /**
