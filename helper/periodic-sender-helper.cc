...
PeriodicSenderHelper::PeriodicSenderHelper ()
{
  ...
  //added by BC // init at 0
  m_randomTimeDelay = 0;
  m_isTxVariable = false;
  m_isBehaviourTimeBased = false;
}
...
PeriodicSenderHelper::InstallPriv (Ptr<Node> node) const
{
  ...
  // added by BC
  app->SetRandomPeriodDelay(m_randomTimeDelay);
  app->SetTimedBehaviour(m_isBehaviourTimeBased);
  app->SetVariableTX(m_isTxVariable);
...
//Added by BC for random variable TX time
void
PeriodicSenderHelper::SetRandomPeriodDelay (double randTimeDelay)
{
  m_randomTimeDelay = randTimeDelay;
}

void
PeriodicSenderHelper::SetVariableTX (bool varTx)
{
 m_isTxVariable = varTx;
}

void
PeriodicSenderHelper::SetTimedBehaviour (bool timedBeh)
{
