...
ClassAEndDeviceLorawanMac::SendToPhy (Ptr<Packet> packetToSend)
{
  ...
  /////////////////////////////////////////////////////////
  ///////////////////
  std::ofstream sendEvents; // added by BC
  //std::ofstream outputSingleNode; // added by BC
  std::string filename;
  //filename = "Node " + std::to_string(m_phy->GetDevice()->GetNode()->GetId());

  sendEvents.open ("sendEvents.txt", std::ofstream::out | std::ofstream::app);
  //outputSingleNode.open (filename.c_str(), std::ofstream::out | std::ofstream::app);
  ///////////////////
  ...
  // added by BC (no longer needed
  /*
  if (params.sf == 11 || params.sf == 12)	//Added by BC
  params.lowDataRateOptimizationEnabled = 1;
*/
  ...
  NS_LOG_DEBUG ("PacketToSend: " << packetToSend);
  double channelFreq = txChannel->GetFrequency (); //added by BC so that i don't have to call this function again and reset things
  //added by BC for debug
  NS_LOG_DEBUG ("m_phy->Send About to start!");

  m_phy->Send (packetToSend, params, channelFreq, m_txPower);    // MODIFIED BY BC: was m_phy->Send (packetToSend, params, txChannel->GetFrequency (), m_txPower);
  ...
  //added by BC for debug
  NS_LOG_DEBUG ("m_phy->Send finished!");
  ...
  m_phy->GetObject<EndDeviceLoraPhy> ()->SetFrequency (channelFreq); //modified by BC, was m_phy->GetObject<EndDeviceLoraPhy> ()->SetFrequency (txChannel->GetFrequency ());
  ...
  // Added by BC for logging
  std::cout.precision(0);
  sendEvents   << "Node: " << m_phy->GetDevice()->GetNode()->GetId()
		  << ", time: " << std::fixed <<  Simulator::Now ().GetSeconds()
		  << ", SF: " << (int)params.sf
		  << ", codingRate: " << (int)params.codingRate
		  << ", bandwidthHz: " << params.bandwidthHz
		  << ", Channel: " << channelFreq
		  << ", TX Power: " << m_txPower
		  << ", Time-on-Air: " << duration
		  << ", Packet: " << packetToSend->GetUid()
		  << std::endl;

  sendEvents.close(); // added by BC
...
ClassAEndDeviceLorawanMac::Receive (Ptr<Packet const> packet)
{
  ...
  std::ofstream downlinkReceived; // added by BC
      ...
      if (messageForUs)
        {
          ...
          downlinkReceived.open ("downlinkReceived.txt", std::ofstream::out | std::ofstream::app);
          // added by BC for logging downlinks
          downlinkReceived   << "Node " << m_phy->GetDevice()->GetNode()->GetId()
        		  << "correctly received a downlink packet intended for it at time "
        				  << std::fixed <<  Simulator::Now ().GetSeconds()
        		  << std::endl;
          downlinkReceived.close();
