...
EndDeviceLorawanMac::Send (Ptr<Packet> packet)
{
  ...
  if (netxTxDelay != Seconds (0))
    {
	  //Modified by BC
	  // don't postone anything, leave next trasmission as it is
      //postponeTransmission (netxTxDelay, packet);
	  NS_LOG_INFO ("Cannot send because duty cycle limitation, packet skipped, data has been lost."); // need to log this someplace else
      return;
    }
  ...
  if (packet != m_retxParams.packet)
    {
      ...
      else if (m_maxNumbTx > 1) //Unconfirmed packet and NbTrans is configured to be more than 1
      {
          ...
          /*std::cout   << "Node: " << m_phy->GetDevice()->GetNode()->GetId()
          << ", time: " << Simulator::Now ()
          << ", SF: " << (int)params.sf
          << ", codingRate: " << (int)params.codingRate
          << ", bandwidthHz: " << params.bandwidthHz
          << ", Channel: " << txChannel->GetFrequency ()
          << ", TX Power: " << m_txPower
          << ", Time-on-Air: " << m_phy->GetOnAirTime (packetToSend, params)
          << ", Packet: " << packetToSend
          << std::endl;
						*/
        }
...
// Added by BC to set or disable ADR on individual node
bool EndDeviceLorawanMac:: GetADREnable (void)
{
	  NS_LOG_FUNCTION (this);
	return m_controlDataRate;
}
void EndDeviceLorawanMac:: SetADREnable (bool enable)
{
	  NS_LOG_FUNCTION (this);
	m_controlDataRate = enable;
}
