          ...
          if (dstNetDevice != 0)
            {
              ...
              // HARD CODED WAY TO ONLY SAVE THE RECEIVE POWER AT THE GW BASED ON KNOWLEDGE OF NUMBR OF PACKETS PRESENT
              if (dstNode >=20){
              // logging added by BC to check RSSI
              std::ofstream packetPower;
              packetPower.open ("packetsRSSI.txt", std::ofstream::out | std::ofstream::app);
              packetPower << "Packet " << packet->GetUid() << " arriving to device " << dstNode << " with RSSI " << rxPowerDbm << std::endl;
              packetPower.close();
              }
            }
          else
