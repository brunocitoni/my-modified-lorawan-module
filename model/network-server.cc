                        ...
                        uint16_t protocol, const Address& address)
{
  ...
  //std::cout << "Receive from forw to NS. Original: " << packet->GetUid() << " Copy: " << myPacket->GetUid() << std::endl; //debug BC

  // Fire the trace source
  m_receivedPacket (packet);

  // logging added by BC
  std::ofstream packetsOutcomeAtNS;
  packetsOutcomeAtNS.open ("packetsOutcomeAtNS.txt", std::ofstream::out | std::ofstream::app);
  packetsOutcomeAtNS << "Packet " << myPacket->GetUid() << " arrived at NS from device " << device<< std::endl;
  packetsOutcomeAtNS.close();
